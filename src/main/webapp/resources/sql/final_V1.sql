...
/* 가입일 */
    ...
    CONSTRAINT FK_DOG FOREIGN KEY (DOG_ID) REFERENCES DOG_T (DOG_ID) ON DELETE CASCADE
);
...
CREATE TABLE INVITE_T(
    ...
    RECEIVER_ID  NUMBER,
    ACCEPTED     NUMBER, --0:no 1:yes
    CONSTRAINT PK_INVITE PRIMARY KEY (INVITE_ID,SENDER_ID,RECEIVER_ID),
    CONSTRAINT FK_SERDER_ID FOREIGN KEY (SENDER_ID) REFERENCES USER_T (USER_ID),
    CONSTRAINT FK_RECIEVER_ID FOREIGN KEY (RECEIVER_ID) REFERENCES USER_T (USER_ID)
);

ALTER TABLE INVITE_T RENAME COLUMN RECIEVER_ID TO RECEIVER_ID;
COMMIT;
...
ALTER TABLE DOG_T ADD AVATAR       VARCHAR2(200 BYTE);
COMMIT;
